...
// Output:
//   last 	- index of the largest element that is smaller than the pivot
//
...
// The process also checks that its list has values less than or equal to
...
void HyperCube_Class::check_list()
{
    ...
    // Check that the local list is sorted and that elements are less than
...
void HyperCube_Class::HyperCube_QuickSort()
{
    ...
    int list_size_ge;  // Number of elements <= pivot
    ...
    for (k = dimension; k > 0; k--)
    {
        ...
        // Search for smallest element in list which is greater than pivot
        // Upon return:
        //   list[0 ... idx-1] >= pivot
        ...
        if (nbr_k > my_id)
        {
            // MPI-2: Send number of elements lower than pivot
            ...
            // Merge local list of elements greater than or equal to pivot with neighbor's list
            ...
            delete[] nbr_list;
            list = new_list;
            list_size = list_size_ge + nbr_list_size;
        }
        else
        {
            ...
            // ***** Add MPI call here *****
            MPI_Send(&list_size_ge, 1, MPI_INT, nbr_k, 0, MPI_COMM_WORLD);

            // Allocate storage for neighbor's list
            nbr_list = new int[nbr_list_size];

            // MPI-8: Receive neighbor's list of elements that are greater than or equal to the pivot
            ...
            // ***** Add MPI call here *****
            MPI_Send(&list[0], list_size_ge, MPI_INT, nbr_k, 0, MPI_COMM_WORLD);

            // Merge local list of elements less than pivot with neighbor's list
